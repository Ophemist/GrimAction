...
int main()
{
    try
    {
        ...
        memory.put_pointer(ui, 0x20, 0x60000000);      // unmapped: skipped
        ...
        const auto length = gdtpc::capture_ui_probe(memory, engine, ui, mark, limits, buffer.data(), buffer.size());
        require(length > 0, "a readable engine and UI produced no record");
        ...
        require(blocks.size() == 4, "expected engine, ui, and exactly two followed children");
        ...
        auto capped = gdtpc::capture_ui_probe(memory, engine, ui, mark, limits, buffer.data(), buffer.size());
        require(parse(buffer, capped).size() == 3, "max_children did not cap followed pointers");
        limits.max_children = 8;

        // A faulting child copy is skipped; the record stays well formed.
        memory.fault_on_copy = child;
        buffer.assign(gdtpc::ui_probe_record_capacity(limits), 0);
        auto faulted = gdtpc::capture_ui_probe(memory, engine, ui, mark, limits, buffer.data(), buffer.size());
        require(faulted > 0 && parse(buffer, faulted).size() == 3, "a faulting child broke the record");
        memory.fault_on_copy = 0;

        // Refusals: unreadable roots, invalid root pointers, and an undersized buffer write nothing.
        buffer.assign(gdtpc::ui_probe_record_capacity(limits), 0);
        require(gdtpc::capture_ui_probe(memory, 0x70000000, ui, mark, limits, buffer.data(), buffer.size()) == 0,
            "an unreadable engine was captured");
        require(gdtpc::capture_ui_probe(memory, engine, 0x70000000, mark, limits, buffer.data(), buffer.size()) == 0,
            "an unreadable UI object was captured");
        require(gdtpc::capture_ui_probe(memory, engine, 0, mark, limits, buffer.data(), buffer.size()) == 0,
            "a null UI pointer was captured");
        require(gdtpc::capture_ui_probe(memory, 0xFFFF800000000000ULL, ui, mark, limits, buffer.data(), buffer.size()) == 0,
            "a kernel-range engine pointer was captured");
        require(gdtpc::capture_ui_probe(memory, engine, ui, mark, limits, buffer.data(), buffer.size() - 1) == 0,
            "an undersized buffer was written");
        require(gdtpc::capture_ui_probe(memory, engine, ui, mark, limits, nullptr, buffer.size()) == 0,
        ...
        std::cout << "PASS: UI probe record layout, root capture with region clamping, heap-only child pointer "
                     "following with alignment, duplicate, root, image, unmapped and scan-range rejection, child cap, "
                     "faulting-copy skip, and refusal of unreadable or invalid roots and undersized buffers.\n";
        return 0;
    }
